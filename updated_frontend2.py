...
FIELDS = [
    "name", "profile_url", "address", "phone", "website", "instagram", "email",
    "num_followers", "num_likes", "num_comments", "insights"
]

def fetch_google_search(query: str, gl: str = "in", num: int = 10, page: int = 1):
    payload = {"q": query, "gl": gl, "num": num, "page": page}
    headers = {"X-API-KEY": SERPER_API_KEY, "Content-Type": "application/json"}
    try:
        with httpx.Client() as client:
            response = client.post(
                f"{SERPER_API_BASE}/search", headers=headers,
...
def get_clinic_socials(clinic_name: str):
    ...
    return {
        "name": clinic_name,
        "profile_url": facebook_url or "",
        "address": "",
        "phone": "",
        "website": "",
        "instagram": instagram_url or "",
        "email": "",
        "num_followers": "",
        "num_likes": "",
        "num_comments": "",
...
def get_facebook_profile_dict(profile_url: str):
    ...
    result = {
        ...
        "instagram": "",
        "email": "",
        "num_followers": profile.get("fan_count", "") 
            or profile.get("followers_count", "") 
            or profile.get("followers", ""),
        "num_likes": profile.get("likes_count", "") 
            or profile.get("likes", ""),
        "num_comments": profile.get("comments", ""),
        "insights": ""
    }
    for item in profile.get("about_public", []):
        ...
        if re.search(r"\+?\d[\d\s-]{7,}", text):
            result["phone"] = text
        elif "@" in text and "." in text and not url:
            result["email"] = text
        elif re.search(r"Bhawani Singh lane|Scheme|Jaipur|India|address", text, re.I):
            result["address"] = text
        elif url and "instagram.com" in url:
            result["instagram"] = url
        elif ("com" in text and url) or (url and url.startswith("http")) and not result["website"]:
            result["website"] = url
    return result
...
def get_instagram_profile_dict(instagram_url_or_username: str):
    ...
    result = {
        ...
        "num_followers": data.get("followers", ""),
        "num_likes": data.get("latest_post_likes", ""),
        "num_comments": data.get("latest_post_comments", ""),
        "insights": data.get("insights", "")
    }
    return result

st.title("Bulk Clinic & Social Media Scraper")
